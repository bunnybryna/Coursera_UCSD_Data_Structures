...
double get_optimal_value(int capacity, vector<int> weights, vector<int> values) {
  ...
  sort(vPerUnit.begin(),vPerUnit.end(),std::greater<double>());
  /*for(std::vector<double>::iterator it=vPerUnit.begin();it!=vPerUnit.end();it++)
    {
  ...
  for (int i=0;i<vPerUnit.size();i++)
  {
      ...
      else
      {
          ...
          //note now valuePerUnit is not vPerUnit[item], but itemValue[item],because vPerUnit has been sorted and its order has changed
          value = value + itemValue[item] * capacity;
          capacity = 0;
          //std::cout << "capacity is "<< capacity << " weight is " << weight << " value is " << value<< std::endl;
          return value;
      }
